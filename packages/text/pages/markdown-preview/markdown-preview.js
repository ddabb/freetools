...
Page({
  ...
  onLoad() {
    wx.setNavigationBarTitle({ title: 'Markdown 预览�? });
  ...
  copyMarkdown() {
    ...
    wx.setClipboardData({ data: text, success: () => utils.showSuccess('Markdown 已复�?) });
  },

  copyHtml() {
    const html = this.data.renderedHtml;
    if (!html) { utils.showText('暂无内容可复�?); return; }
    wx.setClipboardData({ data: html, success: () => utils.showSuccess('HTML 已复�?) });
  },

  refreshPreview() {
    this.renderMarkdown(this.data.markdownText);
    utils.showSuccess('已刷�?);
  },
